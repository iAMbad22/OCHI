...
function Navbar() {
  ...
  return (
    <div className={`fixed z-[999] text-black w-full px-20 py-8 flex justify-between font-["Neue_Montreal"]  backdrop-blur-sm items-center transition-transform ${ isScrolledDown ? 'transform translate-y-[-100%]' : ''}`}>
        <div className='logo'>
        ...
        <div className='links flex gap-10 relative over'>
            {["Services","Our Work","About Us","Insights","Contact"].map((item,index) => (
                <div className='relative group overflow-hidden'>
                  <a key={index*4 } className={`text-lg font-light text-transparent ${ index === 4 && "ml-32"}`}>{item}</a>
                  <a key={index*4 +1} className={`absolute group-hover:top-[-100%] top-0 duration-300 left-0  text-lg font-light   ${ index === 4 && "ml-32"}`}>{item}</a>
                  <a key={index*4 +2} className={`absolute group-hover:top-0 top-full duration-300  left-0 text-lg font-light   ${ index === 4 && "ml-32"}`}>{item}</a>
                  <span className={`${ index === 4 ? "" : "w-full h-[1px] rounded-full bg-black absolute left-[-100%] top-[90%] group-hover:left-0 duration-200"}`}></span>
                </div>
            ))}
        </div>
    </div>
